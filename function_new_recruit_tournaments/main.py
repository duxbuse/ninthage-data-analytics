...
def get_tournaments(start:str = "", end:str = "now") -> list[dict]:
    ...
    response = requests.post(url, json=body, headers={"Accept":"application/json", "User-Agent": "ninthage-data-analytics/1.1.0"})
    data = response.json()
    return data
...
def function_new_recruit_tournaments(request: Request):
    ...
    for event in all_events.tournaments:
        data = {
            "name": event.name or event.short or f"New Recruit Tournament - ${event.id}",
